...
import hbi.core.parsesoap.ResultDes;
...
import java.util.List;
    ...
    ,@RequestParam String name,@RequestParam String type) {
         ...
         * @return
         */
        @RequestMapping(value = "/bom/revise/submitUpdate")
        @ResponseBody
        public ResponseData submitUpdate(HttpServletRequest request,@RequestBody List<Revise>dto)
        {
            IRequest requestContext = createRequestContext(request);
            List<Revise> list=new ArrayList<>();
            String param="{revise:[";
            for (int i = 0; i < dto.size(); i++) {
                if(i==dto.size()-1)
                    param+="{oid:'"+dto.get(i).getOid()+"',number:"+dto.get(i).getNumber()+",name:"+dto.get(i).getName()+",type:"+dto.get(i).getType()+"}";
                else
                    param+="{oid:'"+dto.get(i).getOid()+"',number:"+dto.get(i).getNumber()+",name:"+dto.get(i).getName()+",type:"+dto.get(i).getType()+"},";
            }
            param+="]}";
            System.out.println(param);
            String[] result=(String[])new SoapUtil().request("revise","plmadmin","abc.1234", XMLType.XSD_STRING,param);
            ResponseData data=new ResponseData();
            if(result[0].contains("result\":-2"))
            {
                data.setSuccess(false);
                data.setMessage("接口错误");
            }else  if(result[0].contains("result\":-1")) {
                Gson gson=new Gson();
                ResultAfter after=gson.fromJson(result[0],ResultAfter.class);
                data.setRows(after.getDes());
                data.setSuccess(false);
                data.setMessage(after.getDes().get(0).getDescription());
            }else
            {
                Gson gson=new Gson();
                ResultAfter after=gson.fromJson(result[0],ResultAfter.class);
                data.setSuccess(true);
                data.setRows(after.getAfter());
                list=after.getAfter();
                data.setMessage("所有对象修订成功");
                Long info=0L;
                if(dto.size()>0)
                    info=dto.get(0).getChginfoId();
                for (Revise r:list
                     ) {
                    r.setChginfoId(info);
                    service.insertSelective(requestContext,r);
                }
            }
            /**
             * 将对象转为Revise存到本地数据库！
