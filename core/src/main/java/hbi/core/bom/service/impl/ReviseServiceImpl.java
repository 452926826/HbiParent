...
public class ReviseServiceImpl extends BaseServiceImpl<Revise> implements IReviseService{
    ...
    public void insterExect(IRequest request,List<List<String>>excelList,Long chginfoId) {
        ...
        for (int i=0;i<excelList.size();i++) {
            ...
            revise.setNumber(list.get(1).trim());//编号
            revise.setName(list.get(2).trim());//名称
            revise.setType(list.get(3).trim());//类型
            revise.setVersion((list.get(4)).trim());//版本
            revise.setState((list.get(5)).trim());//状态
            revise.setModifyer((list.get(6)).trim());//修改者
